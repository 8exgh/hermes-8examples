#!/usr/bin/env bash
# Pushes worker-committed website changes upstream, every 2 minutes from cron.
#
# Worker containers have git but no ssh client, so the deploy key lives
...
# when the worker's tree is clean, and push whatever the worker committed.
# The clone itself sits inside the tenant's bind-mounted workspace
# (tenants/<t>/data/workspace), named by tenants/<t>/site_repo_path.
set -u

BASE="$HOME/managed-hermes/tenants"
LOG="$HOME/managed-hermes/site-sync.log"
...
for KEY in "$BASE"/*/site_deploy_key; do
  ...
  DIR="$TDIR/data/workspace/$(cat "$TDIR/site_repo_path")"
  [ -d "$DIR/.git" ] || continue
  ...
  if [ "$AHEAD" -gt 0 ]; then
    STATUS="$TDIR/data/workspace/website-deploy-status.md"
