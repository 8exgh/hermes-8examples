...
import { tenantDataDir } from '../store.js';
...
/**
 * Append the nudge to the tenant workspace where the agent's managed heartbeat
 * cron job picks it up (see templates/workspace/HEARTBEAT.md). Append-only so
 * we never clobber the agent's own edits to the file.
 */
export function deliverToWorkspace(tenant: Tenant, nudge: NudgeRecord): void {
  const dir = path.join(tenantDataDir(tenant.id), 'workspace', 'nudges');
