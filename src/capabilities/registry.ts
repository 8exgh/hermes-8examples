 ...
 * switch on for a customer: what it patches into the worker's config.yaml,
 * which secrets it needs, the instructions the agent gets when it's on, and
 * the nudge copy used to sell it when it's off.
 *
 * Same shape as the OpenClaw fleet registry; the config patches target
 * Hermes' config.yaml (platforms.webhook routes etc.) instead of
 * openclaw.json.
 */
export interface CapabilityDef {
  ...
  /** Container-environment secrets the operator (or customer) must fill in tenants/<id>/.env. */
  env: { key: string; description: string }[];
  /** Hermes-side secrets written to tenants/<id>/data/.env ($HERMES_HOME/.env). */
  hermesEnv?: { key: string; description: string; generate?: () => string }[];
  /** Deep-merged into the tenant's config.yaml when enabled. */
  configPatch: (tenant: Tenant, secrets: Record<string, string>) => Record<string, unknown>;
  /** Written to data/workspace/capabilities/<id>.md when enabled. */
  workspaceDoc: string;
...
export const CAPABILITIES: CapabilityDef[] = [
  {
    ...
    // workspace/mailboxes/*.md (written by the issue-claw-mailbox workflow).
    env: [],
    configPatch: () => ({}),
...
`,
  ...
  },
  {
    ...
    priority: 2,
    env: [
      { key: 'HERMES_GCAL_CREDENTIALS', description: 'Google Calendar OAuth credentials for the calendar integration' },
    ],
    // Calendar wiring is workspace-doc driven until a concrete calendar skill
    // is installed for the tenant (Hermes bundles google-workspace; it needs
    // per-tenant OAuth which this capability hands the agent the doc for).
    configPatch: () => ({}),
...
`,
  ...
  },
  {
    ...
    priority: 4,
    env: [
      { key: 'PHONE_GATEWAY_URL', description: 'Base URL of the phone-call gateway (e.g. https://phone-gateway.fusenv.com)' },
      { key: 'PHONE_GATEWAY_API_KEY', description: 'Per-client bearer token for the gateway (minted by the gateway admin)' },
    ],
    hermesEnv: [
      {
        key: 'WEBHOOK_PHONE_SECRET',
        description: 'Plain secret the phone gateway presents as X-Gitlab-Token on every wake-up ping',
        generate: () => randomSecret(),
      },
    ],
    // Phone-gateway pings arrive on Hermes' webhook platform: POST
    // /webhooks/phone (X-Gitlab-Token = WEBHOOK_PHONE_SECRET, a plain-secret
    // match the adapter supports natively) wakes the agent with the gateway's
    // self-describing `message` field immediately instead of waiting for a
    // heartbeat. Phone work needs real tools, so the route is granted the
    // terminal/file/web toolsets instead of the constrained webhook default.
    configPatch: (_tenant, secrets) => ({
      platforms: {
        webhook: {
          enabled: true,
          extra: {
            ...
            routes: {
              phone: {
                secret: secrets.WEBHOOK_PHONE_SECRET ?? 'changeme',
                prompt:
                  'Phone gateway event ({event}). Read /opt/data/workspace/capabilities/phone.md for the contract, then act on this immediately:\n\n{message}',
                toolsets: ['terminal', 'file', 'web', 'skills', 'memory'],
                deliver: 'log',
              },
            },
          },
...
(add it to each curl below; examples omit it for brevity). Both variables are
in your process environment; read them with the terminal tool (printenv).
...
the line while you fulfill them. The gateway also wakes you directly: every
"tool.requested" / "followup.promised" / "call.inbound.started" / "call.ended"
event is POSTed to your webhook listener and arrives as a new message that
starts with "Phone gateway event". YOUR JOB while every call runs: poll the
...
result. On a followup.promised event:

1. Fetch the orchestration record and append an in-progress entry to
   \`phone/TASKS.md\` (in your workspace) with its orchestration id, callback
   number, requested work, and the exact completion check. Create the file if needed.
...
polling, and the gateway pings your webhook when one starts and ends.
...
- Work in your workspace under \`sites/<project>/\`; keep everything static and simple unless asked.
...
- If a \`website.md\` exists in your workspace, that is YOUR site: follow its workflow (edit, test, commit;
  a trusted publisher pushes and deploys within two minutes).
`,
    offerNudges: [
